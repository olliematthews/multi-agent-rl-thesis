...
class Cyclist:
    def __init__(
        self, env_params, n_cyclists=4, number=0, velocity=0, pose=0, energy=100
    ):
        """
        Initialiser
    ...
    def step(self, action, env_params):
        ...
        self.velocity = max(
            env_params["vel_min"], min(self.velocity, env_params["vel_max"])
        )
    ...
    def print_state(self):
        ...
        """
        print(
            f"Pose is {self.pose}, velocity is {self.velocity}, energy is {self.energy}."
        )
...
class Environment:
    ...
    def step(self, action):
        ...
        self.cyclists = [
            cyclist
            for cyclist, act in zip(self.cyclists, action)
            if not cyclist.step(act, self.env_params)
        ]

        distance = self.order_update_cyclists()
        race_done = (
            self.time >= self.env_params["time_limit"]
            or np.array([cyclist.done for cyclist in self.cyclists]).any()
        )

        self.mean_velocity = (
            1
            / (self.time + 1)
            * np.mean([cyclist.velocity for cyclist in self.cyclists])
            + (1 - 1 / (self.time + 1)) * self.mean_velocity
        )
        self.mean_distance = (
            1 / (self.time + 1) * (distance)
            + (1 - 1 / (self.time + 1)) * self.mean_distance
        )
        ...
        return (
            ...
            self.get_reward(),
            race_done,
            [distance, self.mean_velocity, self.mean_distance],
        )
    ...
    def order_update_cyclists(self):
        ...
        for cyclist in self.cyclists:
            cyclist.rel_poses = [
                c.pose - cyclist.pose for c in self.cyclists if not c == cyclist
            ]
            cyclist.rel_vels = [
                c.velocity - cyclist.velocity for c in self.cyclists if not c == cyclist
            ]
            cyclist.other_energies = [
                c.energy for c in self.cyclists if not c == cyclist
            ]
    ...
    def set_cD(self, cyclist):
        ...
        calc = cD["cDrag"] * (
            ...
            - (
                cD["a"]
                * (pos_poses + cD["offset"])
                / np.exp((pos_poses + cD["offset"]) * cD["b"])
            )
        )
    ...
    def get_state(self):
        ...
        """
        return [
            {"number": cyclist.number, "state": cyclist.get_state(self.time)}
            for cyclist in self.cyclists
        ]
